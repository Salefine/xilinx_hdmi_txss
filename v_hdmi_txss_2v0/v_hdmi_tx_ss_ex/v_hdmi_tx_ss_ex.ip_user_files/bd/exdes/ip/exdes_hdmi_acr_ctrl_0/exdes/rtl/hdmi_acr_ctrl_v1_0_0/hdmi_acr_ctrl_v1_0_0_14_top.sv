...
module hdmi_acr_ctrl_v1_0_0_14_top
(
...
logic        aud_cke;
logic [2:0]  aud_rCKECounter = 'd0;
logic    [2:0]  rCKEDivVal;  
logic    [2:0]  aud_rCKEDivVal;  
...
begin
  ...
  begin
    if (aud_rCKECounter == aud_rCKEDivVal) begin
      aud_rCKECounter <= 0;
      aud_cke       <= 1'b1;
    end
    else 
...
hdmi_acr_ctrl_v1_0_0_lib_pulse_clkcross
PULSE_CLKCROSS_INST
(
  .in_clk   (aud_clk),
  .in_pulse (aud_rPulse),
...
hdmi_acr_ctrl_v1_0_0_lib_data_clkcross
#(
  .pDATA_WIDTH (32)
)
NVAL_CLKCROSS_INST
...
DIVVAL_CLKCROSS_INST
(
  .in_clk  (axi_aclk),
  .in_data (rCKEDivVal),
  .out_clk (aud_clk),
  .out_data(aud_rCKEDivVal)
);
...
endmodule //hdmi_acr_ctrl_v1_0_0_14_top
