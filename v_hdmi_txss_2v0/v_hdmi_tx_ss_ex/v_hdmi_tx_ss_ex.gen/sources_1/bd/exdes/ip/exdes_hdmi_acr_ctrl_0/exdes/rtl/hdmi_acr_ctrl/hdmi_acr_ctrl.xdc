...
# Clock crossing of the CKED value from the HDMI clock to the Audio clock               
set_false_path -from [get_cells -hier -filter {name=~*DIVVAL_CLKCROSS_INST/rIn_Data_reg[*]}] \
               -to   [get_cells -hier -filter {name=~*DIVVAL_CLKCROSS_INST/rOut_Data_reg[*]}]

set_false_path -from [get_cells -hier -filter {name=~*DIVVAL_CLKCROSS_INST/rIn_DValid_reg}] \
               -to   [get_cells -hier -filter {name=~*DIVVAL_CLKCROSS_INST/rOut_DValid_Sync_reg[0]}]

set_false_path -from [get_cells -hier -filter {name=~*DIVVAL_CLKCROSS_INST/rOut_ACK_reg}] \
               -to   [get_cells -hier -filter {name=~*DIVVAL_CLKCROSS_INST/rIn_ACK_Sync_reg[0]}] 
